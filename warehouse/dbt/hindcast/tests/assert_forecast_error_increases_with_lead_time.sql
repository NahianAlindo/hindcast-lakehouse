...
with recent_closed as (
    select milestone_hours, abs_err_temp
...
mae_by_bucket as (
    select milestone_hours, avg(abs_err_temp) as mae_temp
    from recent_closed
    group by milestone_hours
)

select
    (select mae_temp from mae_by_bucket where milestone_hours = 24) as mae_24h,
    (select mae_temp from mae_by_bucket where milestone_hours = 96) as mae_96h
where (select mae_temp from mae_by_bucket where milestone_hours = 96)
    < (select mae_temp from mae_by_bucket where milestone_hours = 24)
