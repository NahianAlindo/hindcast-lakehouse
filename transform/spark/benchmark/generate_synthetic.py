...
def generate(spark: SparkSession, total_rows: int) -> DataFrame:
    ...
    # location_id: 20% of rows land on one of 3 mega-cities, the remaining
    # 80% spread uniformly across the other 9,997 regular cities. A per-row
    # weighted draw (not a nested loop) is what makes the skew exact and
    # scale-independent.
    ...
    # issued_at: one of 90 days x one of 8 fixed 3-hourly issue slots,
    # derived deterministically from row_id so the dataset is reproducible.
    ...
    # timestep_index: 0..39, i.e. +3h increments out to the 5-day horizon.
    timestep_index = F.abs(F.hash(F.col("row_id"), F.lit(SEED + 4))) % TIMESTEPS
    df = df.withColumn(
        "valid_ts", F.col("issued_at") + F.make_interval(hours=timestep_index * F.lit(3))
    ).withColumn("lead_time_minutes", (timestep_index * 180).cast("double"))

    # temp_c: diurnal (hour-of-day) + seasonal (day-of-year) sinusoidal
    # signal plus Gaussian noise -- realistic enough to exercise real
    ...
    df = df.withColumn(
        "temp_c",
        F.lit(15.0)
        + F.lit(10.0) * F.sin(F.lit(2 * 3.141592653589793) * day_of_year / 365.0)
        + F.lit(6.0) * F.sin(F.lit(2 * 3.141592653589793) * hour_of_day / 24.0)
